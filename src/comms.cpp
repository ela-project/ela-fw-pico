 ...
 * @returns Amount of data sent
 */
int Comms::send_buffer_circular(const uint8_t *data, const arrat_index_t offset, const arrat_index_t buffer_len, const arrat_index_t data_length) {
    const size_t first_part_len{buffer_len - offset};
    if (data_length > first_part_len) {
        const size_t second_part_len{data_length - first_part_len};
        send(&data[offset], first_part_len);
        send(&data[0], second_part_len);
    } else {
        send(&data[offset], data_length);
    }
    return 0;
}
 ...
 * @returns Amount of data sent
 */
int Comms::send_buffer(const char *data, size_t lenght) {
    hw_agent.send(reinterpret_cast<const uint8_t *>(data), lenght);
    return 0;
}
    ...
    switch (m_comms_status) {
        case IDLE_TYPE:
            if (data_avalible() >= ELAP_CMD_TYPE_SIZE) {
                ...
                if (ret == 0) {
                    ...
                    hw_agent.send_debug("Unknown type\n");
                }
            }
        ...
        case IDLE_SUBTYPE:
            if (data_avalible() >= ELAP_CMD_SUBTYPE_SIZE) {
                ...
                if (ret <= ELAP_RET_FAIL) {
                    hw_agent.send_debug("Unknown subtype\n");
                    m_comms_status = IDLE_TYPE;
                } else {
        ...
        case RECEIVING_NEW_COMMAND:
            if (m_data_to_receive == 0 || data_avalible() >= m_data_to_receive) {
                ...
                if (ret <= ELAP_RET_FAIL) {
                    hw_agent.send_debug("CMD Failed\n");
                    m_comms_status = IDLE_TYPE;
                } else {
                ...
                case CMD_SET:
                    switch (m_elap_cmd.subtype) {
                        ...
                        case SUB_PRETRIG_COUNT:
                            ...
                            break;
                        case SUB_PIN_MODE:
                            analyzer_agent.set_pin_mode(m_elap_cmd.data.pin_mode.number, m_elap_cmd.data.pin_mode.mode);
                            break;
                        default:
                            break;
                    }
                ...
                case CMD_GET:
                    switch (m_elap_cmd.subtype) {
                        ...
                        case SUB_METADATA:
                            ...
                            m_elap_cmd.data.metadata.max_samplerate = DEVICE_ID;
                            m_elap_cmd.data.metadata.numof_pins = NUM_OF_DIGITAL_PINS;
                            m_elap_cmd.data.metadata.name = (char *)DEVICE_NAME;
                            m_rx_tx_buffer_index = elap_cmd_to_packet(&m_elap_cmd, m_rx_tx_buffer, 0);
                            send_buffer(m_rx_tx_buffer, m_rx_tx_buffer_index);
                            send_buffer(m_elap_cmd.data.metadata.name, m_elap_cmd.data.metadata.str_size);
                            break;
                    ...
                    break;  // case CMD_GET
                default:
                    hw_agent.send_debug("Invalid command\n");
                    break;
            }  // switch (m_elap_cmd.type)
            m_comms_status = IDLE_TYPE;
            break;
...
int Comms::report_sample_buffer() {
    ...
    m_rx_tx_buffer_index = elap_cmd_to_packet(&m_elap_cmd, m_rx_tx_buffer, 0);
    send_buffer(m_rx_tx_buffer, m_rx_tx_buffer_index);
    // hw_agent.send_sample_buff();
    send_buffer_circular(hw_agent.m_sample_buffer, hw_agent.m_sample_buffer_start, SAMPLE_BUFFER_SIZE, hw_agent.m_sample_buffer_count);
    hw_agent.comm_flush();
    return 0;
}