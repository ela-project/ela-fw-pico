...
class Comms {
   public:
    ...
    inline static int send(const uint8_t *data, const size_t length) {
        return hw_agent.send(data, length);
    };
    ...
    static int send_buffer_circular(const uint8_t *data, const arrat_index_t offset, const arrat_index_t buffer_len, const arrat_index_t data_length);
    static int send_buffer(const char *data, size_t length);

    template <size_t LENGTH>
    static int send_buffer(const char (&data)[LENGTH]) {
        send_buffer(data, LENGTH);
        return 0;
    };
   ...
   private:
    ...
    inline static void _uint_to_bytes(uint8_t *buffer, uint64_t num, int buffer_offset, int num_of_bytes) {
